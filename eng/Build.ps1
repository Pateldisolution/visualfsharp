...
function VerifyAssemblyVersions() {
    ...
    if (Test-Path $fsiPath) {
        ...
        "#r @""$env:USERPROFILE\.nuget\packages\Newtonsoft.Json\9.0.1\lib\net45\Newtonsoft.Json.dll""" | Out-File "$asmVerCheckPath\AssemblyVersionCheckDependencies.fsx"
        Exec-Console $fsiPath """$asmVerCheckPath\AssemblyVersionCheck.fsx"" -- ""$RepoRoot\build\config\AssemblySignToolData.json"" ""$ArtifactsDir"""
    }
}
...
function TestUsingNUnit([string] $testProject, [string] $targetFramework) {
    ...
    $args = "test $testProject --no-restore --no-build -c $configuration -f $targetFramework -v n --test-adapter-path . --logger ""nunit;LogFilePath=$testLogPath"""
